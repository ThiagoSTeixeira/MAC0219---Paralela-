...
def run_seq(repetitions=15, size=4096):
    ...
    for _ in range(repetitions):
        res = subprocess.run(['./mandelbrot_seq',
...
def run_omp(repetitions=15, thread_range=[1, 2, 4, 8, 16, 32, 64],
            size=4096):
    ...
    for t in thread_range:
        print("[omp]:", t, "threads")
        for _ in range(repetitions):
            res = subprocess.run(['./mandelbrot_omp',
                                  "-0.188", "-0.012", "0.554", "0.754",
                                  str(size), str(t)],
                                 capture_output=True)
            string += str(t)+','
...
def run_pth(repetitions=15, thread_range=[1, 2, 4, 8, 16, 32, 64],
            size=4096):
    ...
    for t in thread_range:
        ...
        for _ in range(repetitions):
            res = subprocess.run(['./mandelbrot_pth',
                                  "-0.188", "-0.012", "0.554", "0.754",
                                  str(size), str(t)],
                                 capture_output=True)
            string += str(t)+','
...
def run_cuda(repetitions=15, dimensions=[(2, 2), (4, 4), (8, 8), (16, 16), (32, 32)],
             size=4096):
    ...
    for p in process_range:
        for x, y in dimensions:
            ...
            for _ in range(repetitions):
                res = subprocess.run([
                    "mpirun", "-np", str(p), "--host", "localhost:" + str(p),
                    "mandelbrot_ompi_cuda", "-0.188", "-0.012", "0.554",
                    "0.754", str(size), str(x), str(y)],
                    capture_output=True)
